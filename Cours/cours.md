...
1.1 donne l’exemple du pattern Template Method qui sera décrit au chapitre Le pattern Template Method.Dans ce pattern, la méthode calculeMontantTTC invoque la méthode calculeTVA qui est abstraite dans la classe Commande. Elle est définie dans les sous-classes de Commande à savoir les
classes CommandeFrance et CommandeLuxembourg. En effet, le taux de TVA varie en fonction du pays. LA methode calculeMontantTTC est appele la template method. Elle introduit un algorithme basé sur une méthode abstraite.
...
même, le calcul de TVA est donc différent en France et au Luxembourg. Par conséquent, le pattern Template Method
constitue une bonne illustration du polymorphisme.
